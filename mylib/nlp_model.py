...
def find_similar_documents(query_text, tfidf_matrix, doc_list, vectorizer, n=2):
    """
    ...
    return [(doc_list[i], similarity_scores[i]) for i in similar_indices]

def find_similarity(documents, query_text):
    """
    Finds and returns the top similar documents for a given query.

    Args:
        documents (list): List of original documents.
        query_text (str): The search query. Example usage: query_text = "The quick brown fox"
    ...
    similar_docs = find_similar_documents(query_text, tfidf_matrix, documents, vectorizer)

    # Prepare the result string
    result = "\n 🔍 Similar documents to '{}':\n".format(query_text)  # Using .format instead of f-string
...
test_result = find_similarity(test_documents, query)
print(test_result)
